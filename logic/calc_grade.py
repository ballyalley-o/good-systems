...
def calculate_grades(csv_file_path):
    ...
    with open(csv_file_path, 'r') as csv_file:
        ...
        for module_no, grade in grades.items():
            print(Fore.YELLOW + f' 〉Module {module_no}: {grade:.2f} ' + Style.RESET_ALL)

        print('\n')

        sum_grade = sum(grades.values())
        total_grade = sum_grade / len(grades)
        print(Fore.BLACK + Back.WHITE + f' 〉 Average: {total_grade:.2f} ' + Style.RESET_ALL)
        print('\n')